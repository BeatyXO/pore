...
def test_live_create_evidence_resolve_cycle(default_account):
    ...
    print("CREATE_TX", opened.get("hash", opened.get("tx_id")))
    inspection = contract.submit_inspection_report(args=[base, "inspection-hash-001", "inspection complete"]).transact(transaction_context=context())
    print("INSPECTION_TX", inspection.get("hash", inspection.get("tx_id")))
    authorization = contract.authorize_repair(args=[base, "quote-hash-001"]).transact(transaction_context=context())
    print("AUTHORIZATION_TX", authorization.get("hash", authorization.get("tx_id")))
    before = contract.submit_repair_evidence(args=[base, "BEFORE_PHOTO", "https://httpbin.org/image/jpeg", "before photo"]).transact(transaction_context=context())
    print("BEFORE_TX", before.get("hash", before.get("tx_id")))
    ...
    print("AFTER_TX", evidence.get("hash", evidence.get("tx_id")))
    ...
    print("RESOLVE_TX", resolved.get("hash", resolved.get("tx_id")))
    print("FINAL_ACCOUNTING", contract.get_intent(args=[base]).call())
